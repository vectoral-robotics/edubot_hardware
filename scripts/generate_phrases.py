...
Run at Docker build time (after Piper and the voice model are installed):
    ...
    python3 scripts/generate_phrases.py \
        --voice-model /opt/piper/voices/en_GB-alba-medium.onnx \
        --phrases     edubot_hardware/phrases.json \
...
def main() -> None:
    ...
    parser.add_argument(
        "--voice-model",
        default="/opt/piper/voices/en_GB-alba-medium.onnx",
        help="Piper voice model (.onnx)",
    )
    parser.add_argument(
        "--phrases",
        default=str(Path(__file__).parent.parent / "edubot_hardware" / "phrases.json"),
